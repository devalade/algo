...
var import_node4 = __toESM(require_main4(), 1);
...
function getWordAtPosition(document, position) {
  ...
  if (!wordMatch || !wordRestMatch)
    return null;
  const word = wordMatch[0] + wordRestMatch[0];
  return word || null;
}
function computeErrorEndCharacter(document, line, column) {
  const text = document.getText();
  const lines = text.split(/\r?\n/);
...
function setDiagnosticCapability(value) {
  hasDiagnosticRelatedInformationCapability = value;
}
function register(connection, documents) {
  documents.onDidChangeContent((change) => {
    validateTextDocument(change.document, connection);
  });
}
async function validateTextDocument(textDocument, connection) {
...
// packages/lsp-server/src/hover.ts
function register3(connection, documents) {
...
// packages/lsp-server/src/formatting.ts
function register4(connection, documents) {
  connection.onDocumentFormatting((params) => {
    return formatDocument(params, documents);
  });
}
function formatDocument(params, documents) {
  const document = documents.get(params.textDocument.uri);
  if (!document)
    return [];
  const text = document.getText();
...
// packages/lsp-server/src/symbols.ts
var import_node3 = __toESM(require_main4(), 1);
function register5(connection, documents) {
...
function provideDefinition(params, documents) {
  ...
  if (!table)
    return null;
  const symbol = table.symbols.get(word);
  if (!symbol || symbol.line === undefined || symbol.column === undefined)
    return null;
  return {
...
function provideDocumentSymbols(params, documents) {
  const document = documents.get(params.textDocument.uri);
  if (!document)
    return [];
  const table = symbolTables.get(params.textDocument.uri);
  ...
  if (ast && ast.value) {
    ...
    symbols.push({
      name: programmeName,
      kind: import_node3.SymbolKind.Module,
      range: {
        start: { line: 0, character: 0 },
  ...
  if (table) {
    for (const [name, symbol] of table.symbols) {
      ...
      symbols.push({
        ...
        kind: import_node3.SymbolKind.Variable,
        range: {
...
// packages/lsp-server/src/server.ts
var connection = import_node4.createConnection(import_node4.ProposedFeatures.all);
...
var hasDiagnosticRelatedInformationCapability2 = false;
connection.onInitialize((params) => {
  const capabilities = params.capabilities;
  hasConfigurationCapability = !!(capabilities.workspace && !!capabilities.workspace.configuration);
  hasWorkspaceFolderCapability = !!(capabilities.workspace && !!capabilities.workspace.workspaceFolders);
  hasDiagnosticRelatedInformationCapability2 = !!(capabilities.textDocument && capabilities.textDocument.publishDiagnostics && capabilities.textDocument.publishDiagnostics.relatedInformation);
  setDiagnosticCapability(hasDiagnosticRelatedInformationCapability2);
  const result = {
    capabilities: {
...
connection.onInitialized(() => {
  if (hasConfigurationCapability) {
    connection.client.register(import_node4.DidChangeConfigurationNotification.type, undefined);
  }
  if (hasWorkspaceFolderCapability) {
...
register(connection, documents);
register2(connection, documents);
register3(connection, documents);
register4(connection, documents);
register5(connection, documents);
